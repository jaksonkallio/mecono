...
public class NodeChain {
	...
	public NodeChain(ArrayList<Node> stops) {
		this.stops = stops;
	}

	public NodeChain(NodeChain copy_path) {
		this.stops = copy_path.getStops();
	}

	public NodeChain(NodeChain concat_path1, NodeChain concat_path2) {
	...
	public boolean equals(Object o) {
		if (o instanceof NodeChain) {
			NodeChain other = (NodeChain) o;
	 ...
	 */
	public NodeChain getSubpath(int start, int end) throws BadPathException {
		...
		return new NodeChain(subpath_stops);
	}
	 ...
	 * @return
	 */
	public NodeChain getSubpath(int end) {
		try {
	 ...
	 * @return
	 */
	public NodeChain getIntermediatePath() {
		try {
	...
	public static ArrayList<NodeChain> convertToRawPaths(ArrayList<Path> outwards_paths) {
		ArrayList<NodeChain> paths_raw = new ArrayList<>();
		for (Path outward_path : outwards_paths) {
			paths_raw.add(outward_path.getNodeChain());
		}
		return paths_raw;
	}
